import { system, world, CustomCommand, CommandPermissionLevel, CustomCommandParamType, StartupEvent, CustomCommandResult, Player, Vector3, CustomCommandOrigin, ItemStack, GameMode, } from "@minecraft/server";
...
export default class TPCommand {
  static setup(init: StartupEvent) {
    const tpSetCommand: CustomCommand = {
      ...
      optionalParameters: [
        { type: CustomCommandParamType.Location, name: "loc" }
      ]
    };
...
function showTPSetForm(player: Player) {
  const form = new ModalFormData()
    .title(`Set teleportion to ${vStr(player.location)}`)
